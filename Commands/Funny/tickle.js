...
module.exports = {
    ...
    execute(message, args, commandName, client, Discord) {
        ...
        if (!user){
            ...
            .setColor("RANDOM")
            .setDescription("**Hãy tag một người bạn nào đó để thọc léc :D**")
            return message.channel.send({embeds: [CantSend]})
        }
        if (message.author === user) {
            const SendButNoone = new MessageEmbed()
            .setColor("RANDOM")
            .setDescription("**Bạn không thể tự thọc léc chính mình**")
        ...
        superagent.get('https://nekos.life/api/v2/img/tickle')
            .end((err, response) => {
                const embed = new MessageEmbed()
                    .setTitle(`${message.author.username} đang thọc léc ${user.username}`)
                    .setFooter(`hahahahahaha`)
